...
// Initialize Socket.io connection to Render backend for multi-client sharing
const BACKEND_URL = "https://arduino-object-radar.onrender.com";
let socket = null;
try {
    socket = io(BACKEND_URL);
    socket.on('connect', () => console.log('Connected to Cloud Relay'));
    
    // Listen for incoming radar data shared by other users
    socket.on('update-radar', (data) => {
        // Ignore cloud data if this browser is already physically connected to an Arduino
        if (liveData.connected && serialPort) return;
...
function updateConnectionUI(connected, customStatusText = null) {
    if (connected) {
        elConnection.textContent = '● Arduino Connected (Web Serial)';
...
async function connectArduino() {
    if (!("serial" in navigator)) {
        alert("Use Google Chrome or Microsoft Edge for Web Serial support.");
        return false;
    }
    try {
        ...
        serialPort = ports.length ? ports[0] : await navigator.serial.requestPort();
        ...
        showToast(`Connected to Arduino USB`);
        ...
        showToast("Connection failed. Check USB connection.");
        return false;
    }
}
...
// Start visual loop & trigger auto-connect on load if already permitted
animationLoop();
// Attempt silent reconnect if permission was previously granted
navigator.serial?.getPorts().then(ports => {
    if (ports.length > 0) connectArduino();
});
